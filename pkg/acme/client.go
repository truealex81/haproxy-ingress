...
func (c *client) ensureAccount() error {
	if _, err := c.client.GetAccount(c.ctx); err != nil {
		...
		if ok && acmeErr.Type == acmeErrAcctDoesNotExist {
			...
			c.logger.Info("terms agreed, new account created on %s", c.endpoint)
		} else {
			return err
		}
	}
	// TODO update contact if needed
	return nil
}
			...
			if challenge.Type == acmeChallengeHTTP01 {
				...
				if err != nil {
					if acmeErr, ok := err.(acme.AuthorizationError); ok {
						...
						return fmt.Errorf("authorization error: domain=%s status=%s", auth.Identifier.Value, acmeErr.Authorization.Status)
					}
					return err
				}
			}
